...
main() {
  ...
  for (m = 0; m <= NN; m++) {
    ...
    if(prob >= THR)
      /*      sp_misc_create_sp(sp, &sim, 1, SM_1STORDER, LS_NORMAL+LS_POWDER, (void *) 0, 0, 0, I_INTERP_AXIAL, 8); */
      sp_misc_create_sp(sp, &sim, 1, SM_1STORDER, LS_NORMAL, (void *) 0, 0, 0, I_INTERP_AXIAL, 8);
  }
#else
  for (m = 0; m <= NN; m++)
    for (n = 0; n+m <= NN; n++) {
